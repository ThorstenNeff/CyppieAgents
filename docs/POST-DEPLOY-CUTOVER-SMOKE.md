...
| 1.1b [BLOCK] | **Login-Redirect TERMINIERT (keine Loop) — CYP-515-Regressions-Guard.** Der unauth-Redirect macht **beschränkt** viele Hops zu Kratos und **settelt** (finaler 200 Login-Seite). | Ein Redirect-**Loop** (neue flow-id je Hop) erschöpft das Follow-Limit → `num_redirects` am Cap + non-2xx-Final = FAIL. **Das ist der [BLOCK], den der Auftraggeber beim CYP-515-Cutover manuell traf** — jetzt in `deploy-smoke-headers.sh` (§1.1b) automatisiert, damit ein Re-Cut vor einem Menschen loopt. (HTTP-Level; ein reiner Client/JS-Redirect-Loop braucht den Browser-Schritt.) | Rig hatte keinen Kratos-Login-Flow. |
