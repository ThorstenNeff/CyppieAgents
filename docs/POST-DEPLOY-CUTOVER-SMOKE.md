...
| 1.1b [BLOCK] | **Login-Redirect terminiert (keine HTTP-Loop) — CYP-515-Guard, HTTP-Loop-Klasse.** Der unauth-Redirect macht **beschränkt** viele HTTP-Hops zu Kratos und **settelt** (finaler 200). | Eine HTTP-**Loop** (neue flow-id je 30x-Hop) erschöpft das Follow-Limit → `num_redirects` am Cap + non-2xx = FAIL. `deploy-smoke-headers.sh` (§1.1b). **⚠️ HTTP-only:** CYP-515 selbst ist ein **JS-Loop** (`redirectToLogin()` via `window.location`, keine 30x-Kette) → curl-`-L` folgt dem einen 303 zu `/?flow=`, kriegt SPA-HTML (200), sieht **keine** HTTP-Loop → §1.1b **allein** ließe CYP-515 durch. Die JS-Loop-Klasse fängt **§1.1c**. | Rig hatte keinen Kratos-Login-Flow. |
| 1.1c [BLOCK] | **Unauth JS-Redirect-Loop-Detektor (CYP-515-Klasse, robust).** Headless `/` unauth laden → die Seite **settelt** in einen stabilen Login-Zustand statt N JS-Navigationen im Timeout zu überschreiten. `deploy-smoke/login-loop.spec.ts`. | Zwei markup-agnostische Loop-Signale, **eins failt = FAIL:** (a) Top-Frame-Navigations-**Count** im Fenster > Cap (schnelle Loop) · (b) die Seite geht **nie ruhig** — Navigation noch im End-Tail (langsame Loop). **Das ist der reale CYP-515-Mechanismus** (`window.location`-Loop, kein HTTP-30x). Ergänzt §1.1b: curl=HTTP-Loop · Playwright=JS-Loop. | Rig hatte keinen Client-Auth-Gate. |
...
| 1.3 [BLOCK] | **CSP-Header präsent (B-2, im Rig offen).** Response der SPA trägt `Content-Security-Policy`. | Mind. `default-src 'self'`, `script-src` **nonce-basiert** (`'nonce-…'`, **kein** `unsafe-inline`), `connect-src` = API/WS-Origin, `object-src 'none'`, `frame-ancestors 'none'` + `X-Content-Type-Options: nosniff`. **Aktiv-Probe (Browser):** eine injizierte Inline-`<script>` **ohne** gültige Nonce wird **geblockt** (`securitypolicyviolation` in der Console). Kein CSP-Header / `unsafe-inline` präsent = FAIL. | Rig hatte **keinen** CSP (B-2-Gap). |
| 1.3b [BLOCK] | **Per-Response-Nonce differt.** Zwei Requests an dieselbe URL → **zwei verschiedene** `script-src 'nonce-…'`-Werte (CSP-Nonce ist per-response, nicht statisch). | Gleiche Nonce über 2 Requests = FAIL (statische Nonce ist kein Schutz — dann könnte injiziertes Inline sie wiederverwenden). | Rig: kein CSP. |
| 1.4 [BLOCK] | **Session-Cookie-Flags.** Der Kratos-`Set-Cookie` (Session) trägt `HttpOnly` + `Secure` + `SameSite`. | Per `Set-Cookie`-Header (curl) alle drei Flags gesetzt; `document.cookie` gibt den Session-Cookie **nicht** her (HttpOnly). Fehlt ein Flag = FAIL. | Rig: kein echter Cookie. |
| 1.5 | **`CONTRACT_REQUIRE_REAL` im Cutover-Build.** Die ausgelieferten Typen stammen aus dem **echten** `:core`-Export (kein Provisional-Stub). | Deploy-Log/Build-Artefakt zeigt `CONTRACT_REQUIRE_REAL=1` gesetzt (fehlender Real-Export = fatal). Provisional-Fallback = FAIL. | Rig setzte es im `contract:gen`; hier am Deploy bestätigt. |
