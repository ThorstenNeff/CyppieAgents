...
- **★ Naht zu §3.3 (`CYP-736`):** der Repo-„done" (CLONED_OK) hängt am Clone-Status-Seam — bis der Seam da ist, ist „gespeichert" der höchste §3.2-Repo-State; **das Gesamt-TRANSPARENT (Kern-done) bleibt daher unerreichbar bis §3.3** → der Gate bleibt ohne §3.3 in ACTIVE/Skip-Degraded (honest: nie fälschlich „done"). Mein Clone-Lifecycle-Screen-Spec (5 States, auth-vs-url distinkt, cloning-slow ehrlich) setzt oben drauf.
