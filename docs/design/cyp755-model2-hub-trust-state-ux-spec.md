...
- **Exakter Token** (am gemergten Objekt `hubTrustView.ts` / `index.css` verifiziert):
  ...
  - **CSS-Glyph `.hub-trust-trusted .hub-trust-glyph`: `--md-sys-color-primary` → `--md-sys-color-on-surface`.** `on-surface` ist der **neutrale Baseline-Text-Token, KEIN Akzent** — die Affirmation/Garantie saß in der **Akzent-Hue `primary`** („the ONLY positive state"), nicht im Emphase-Level. `on-surface` trägt null Positiv-/Erfolgs-Semantik → Overclaim entfernt, **ohne** in den gedämpften Absence-Ton zu kippen. `on-surface`-auf-`surface-variant` ist im DS bereits ein AA-Text-Paar (Label nutzt es), Glyph als Graphik ≥3:1 gedeckt.
  - **Per-Surface-Token bewusst ≠ Desktop (PL §5, kein Byte-Zwang) — geteiltes PRINZIP, nicht geteiltes Byte:** Team-1s Desktop (`remote-trust-recovery-ux-spec.md` §2.2, am Objekt gelesen) rendert `TrustResolution.Pinned` = `trustPinned` **`onSurfaceVariant`/„NIE grün"** — ein kleiner Kontext-Strip-Indikator, der **nicht** neben einem `on-surface-variant`-unknown-Badge sitzt (anderes State-Modell: TOFU-Fingerprint-Recovery, nicht das 5-Zustands-Badge). web-ts' 5-Zustands-Badge hat **unknown/pending schon auf `on-surface-variant`** (gemergter Code), also muss `trusted` **eine Stufe hoch** auf `on-surface`, um **emphase-distinkt** zu bleiben — genau PLs geteiltes Prinzip „**neutral + emphase-genug für Distinktion von UNKNOWN/PENDING**". Der exakte Token folgt der **je-Surface-Palette** (wovon `trusted` distinkt sein muss), das Prinzip ist identisch: neutral, nie-grün, Form+Label-getragen. **Minimal:** nur `trusted` bewegt sich; unknown/pending/rejected/stale unangetastet.
...
10. **`trusted` neutral, keine Affirmation (CYP-803)** — beide Richtungen gezahnt: (a) **kein Affirm-Akzent:** `trusted` rendert **`data-tone="neutral"`** (nicht `"positive"`) und Glyph-Token **`on-surface`** (nicht `primary`/Akzent-Hue). *(Mutation: `trusted` → `tone:'positive'` / Glyph `--md-sys-color-primary` → RED = Garantie-Overclaim, derived≠native verletzt.)* (b) `trusted` bleibt **distinct von `unknown`**: Form `●`≠`◯`, Wort „vertraut"≠„Vertrauen nicht geprüft", Glyph-Token `on-surface`≠`on-surface-variant` (emphase-distinkt, nicht in die Absence-Zustände gekippt). *(Mutation: `trusted`-Glyph → `on-surface-variant` (= unknown/pending-Ton) → RED = Über-Neutralisierung, [[reconcile-not-collapse-distinct-states]].)* **Wichtig:** `on-surface` ist Baseline-Neutral, **kein** Akzent — (a) und (b) sind kein Widerspruch (Affirmation = die `primary`-Hue, nicht das Emphase-Level).
