...
> Status: **Design komplett (Rev. 3.5 — PL-§4b-FREIGABE 2026-07-21; A1→Zahn 7(b), A2→Zahn 8(a), B1/B2→§11.1, §14 Bau-Kopplungen. Reviewer-Zahn-Verifikation über 2 Re-Pässe: GO 7(b)/B1/B2/§14 + Faithfulness-GO auf 8(a); 8(a) auf DREI strukturelle Vollständigkeits-Beine gehärtet — ① Typ-Konstruktions-Closure, ② `isOperator`-Call-Site-Pin, ③ Single-Cookie-Consumer-Arch-Assertion (seizure-kritisch, konfiniert rohes Cookie UND `IdentityProvider.whoami` auf `auth/`, mit positiver Detektor-Kontrolle). Typ-Closure allein war notwendig-nicht-hinreichend). Nächstes Gate: Reviewer-Final-OK auf die 3-Bein-Fassung → PL-Go am Volltext (7b+8a) → Modell-2-Bau nach CYP-775-Kette. KEIN Bau vor PL-Go.**
...
7. **★ Aktiver-Widerruf-Wirksamkeit (Q2/a2)** (§7): (a) nach Widerruf → **Push-STALE/REJECTED über ALLE live Hub-Sessions SOFORT** (nicht ≤TTL); CYP-697-Fall (Operator entfernt → Agenten verstummen) explizit. **(b) Registry-Partition-Floor (PL-A1, fail-closed):** fällt die C3-Widerruf-Registry aus / partitioniert, degradiert die Widerruf-Wirksamkeit auf den **≤TTL-lazy-Floor — NIE länger, NIE fail-open**; ein Registry-Ausfall darf die a2-Sofortwirksamkeit nicht über den ≤TTL-Floor hinaus schwächen (sonst untergräbt genau der Ausfall den a2-Zweck). **Strukturell (Reviewer-Schärfung):** der ≤TTL-Floor wird vom **passiven per-Hub-`exp`-Check (`NOT_EXPIRED`) getragen, registry-UNABHÄNGIG** — eine Partition kann den Floor nicht mit-abschalten (⟹ „nie fail-open" ist strukturell erzwungen, nicht bloß behauptet); der Floor = die **bestehende** `exp` der schon ausgestellten Credentials, **kein Re-Mint** (sonst dehnt sich der Floor still). Non-vakuos: Mutation „Registry-down → Widerruf ignoriert (fail-open)" ODER „Registry-down → degradiert über ≤TTL hinaus" ODER „Floor via Re-Mint verlängert" → rötet.
8. **★★ Browser-AAL2-Chokepoint-EIGENSCHAFT + Nicht-Fälschbarkeit der local/remote-Grenze** (§9.3/§9.5): (a) eine synthetische **Nicht-AAL2**-Operator-Cookie-Session wird an JEDER operator-autoritativen Cookie-Kante abgewiesen — **strukturell via Konstruktions-Stellen-Closure über die Operator-Autoritäts-TYPEN (PL-A2, Reviewer-geschärft)**, NICHT über eine Flächen-/Auflöser-Liste. Operator-Autorität ist ein **Typ**-Konstrukt (≥3 Typen an ≥5 Mint-Stellen, teils mit **berechneter** Rolle), kein einzelner Auflöser — die frühere Fassung „`resolvePrincipal` ODER `isOperator`" verfehlt/falsch-klassifiziert `CpJwtVerifier.kt:36` (Tunnel-`Human(OPERATOR)`, device-PoP **NICHT** AAL2). Der Zahn hat **DREI strukturelle Vollständigkeits-Beine** (keine Operator-Autoritäts-Bestimmung entkommt ungepinnt — Typ-Closure allein ist **notwendig, nicht hinreichend**, Reviewer-Re-Pass) **plus das per-Achse-Verhalten** je gepinnter Stelle:

   **Strukturelle Vollständigkeit — drei Beine:**
   - **① Typ-Konstruktions-Closure (Exhaustivität ERZWUNGEN, nicht behauptet):** ein Arch-Scan (compiler/PSI/Konstruktor-Enum) findet JEDE Konstruktions-Stelle der Operator-Autoritäts-Typen — `MachineOperator`, **ALLE `Human(`-Sites mit erreichbarer OPERATOR-Rolle** (nicht nur `Human(OPERATOR)`-Literale: die Rolle ist bei `Principal.kt:121` = `Human(id, ensureAssigned(...))` **berechnet** — ein Literal-Grep verfehlte sie), `TerminalPrincipal.Operator`, künftige — und behauptet **`gefundene Mint-Stellen == gepinnte Registry`**. Eine neue Mint-Stelle (21.-Kante-WS/SSE/MCP-Wrapper, der einen Operator-Typ prägt) → `gefunden ≠ gepinnt` → **ROT**. Hängt am **Typ-Konstruktor** (enumerierbar), NICHT an einer Flächen-Aufzählung → keine vergessene Fläche entkommt still. Liste *behauptet* Vollständigkeit, die Closure *erzwingt* sie.
   - **② `isOperator(...)`-Call-Site-Pin (Token-Achse):** `isOperator(token)` (`Auth.kt:33`, `token==operatorToken`) ist ein **Boolean**, konstruiert KEINEN Prinzipal-Typ (z. B. `AgentSocket:65` `isOperator→true`) → die Typ-Closure ① ist dafür **blind**. Der Test pinnt die `isOperator`-Call-Sites: Registry-Completeness-Hygiene (B2-geschützt, kein Browser-Seizure-Vektor, aber nötig, damit „jede operator-autoritative Fläche gepinnt" wirklich stimmt).
   - **③ ★ Single-Cookie-Consumer-Arch-Assertion (das seizure-kritische Bein):** statischer Source-Tree-Scan (wie ① compile-scan, KEIN Runtime-Zahn) pinnt, dass **ZWEI** Roh-Credential-Primitive nur im `auth/`-`resolvePrincipal`-Pfad konsumiert werden: **(a)** das rohe Cookie/Header (`ory_kratos_session` / `X-Session-Token`) wird NUR in `auth/` referenziert (`KratosIdentityProvider`/`Principal`/`Csrf`), **null `routing/`**; **(b)** der `IdentityProvider.whoami`/`resolveSession`-Call selbst wird NUR aus `Principal.kt` (`resolvePrincipal`) / der Provider-Kette gerufen, **nie direkt aus `routing/`**. Die subtilere Hälfte (b): eine künftige `routing/`-Fläche, die `resolvePrincipal` umgeht und `identityProvider.whoami()` DIREKT ruft + die Rolle prüft, bestimmt Operator aus der Session **OHNE das AAL2-Gate** (das IN `resolvePrincipal` sitzt) — konstruiert keinen Typ (①-blind), ruft nicht `isOperator` (②-blind); (a) allein fängt das nicht, wenn die Fläche über den Provider statt das rohe Cookie geht. **Beide Primitive müssen konfiniert sein.** **Anti-Vakuität (Pflicht):** positive Kontrolle IM selben Test — assertiere ZUERST, dass der Detektor die BEKANNTEN `auth/`-Consumer (`KratosIdentityProvider`+`Principal`) **tatsächlich flaggt**, DANN „und null in `routing/`" (sonst ist ein kaputter Detektor — falscher Symbol-/Cookie-Name — still grün).

   **Per-Achse-Verhalten (Verhaltens-Zahn je gepinnter Stelle):** COOKIE (`Principal.kt:121`) → AAL2-Gate (§9.3) + loopback-Posture (§9.5); **TUNNEL (`CpJwtVerifier.kt:36`) → device-PoP über Noise-`h` (Rr3); AAL2 wird NICHT verlangt** (browser-unerreichbar, kein Cookie-Vektor — ein Test, der hier AAL2 forderte, wäre FALSCH); TOKEN (`Principal.kt:106` / `TerminalAccess.kt:173`) → `isOperator` + B2 (Token nie im Browser); **abgeleitet** (`TerminalAccess.kt:178/180`, re-wrappt ein `resolvePrincipal`-Ergebnis, keine neue Autorität) → erbt. Die **Achsen-Spalte** ist, was `CpJwtVerifier` als device-PoP-statt-AAL2 einordnet → die AAL2-Pflicht wird ihm NICHT fälschlich aufgeworfen.

   **Gepinnte Baseline-Registry (Mensch/PO-gepflegt, Closure beweist ihre Exhaustivität) {Mint-Stelle → (Typ, Achse, Anti-Seizure)}:** `Principal.kt:106`(MachineOperator/TOKEN/lokal-Box+B2) · `Principal.kt:121`(Human-OP berechnet/**COOKIE**/AAL2+loopback) · `CpJwtVerifier.kt:36`(Human-OP/**TUNNEL**/**device-PoP, NICHT AAL2**) · `TerminalAccess.kt:173`(TermOp/TOKEN/`isOperator`) · `:178/:180`(TermOp/**abgeleitet**/erbt) · `TunnelGodTokenGuard`(god-Token, umgeht TTL/Widerruf by design — separat benannt). (b) **Loopback-Bind RUNTIME-Zahn (nicht Build-Default):** Boot/Init mit `HUB_HOST=0.0.0.0` → Browser-AAL2-OPERATOR-Posture DISABLED (fail-closed), via `InetAddress.getByName(host).isLoopbackAddress` (T3-Auflösung, NICHT String-`==127.0.0.1`); pinnt BEIDE — `host="localhost"` → ENABLED (löst loopback) UND `host="0.0.0.0"` → DISABLED — nicht nur den `127.0.0.1`-Fall. (c) **Kein-3.-Ingress-Meta-Test:** genau die 2 loopback-Connectors, kein bare-cookie-fähiger Nebeneingang zu `/api`. Non-vakuos, die Mutationen die röten MÜSSEN: „AAL1 erlaubt an `:121`" (COOKIE-Zahn) / „off-loopback → Browser-OPERATOR trotzdem an" / „String-`==127.0.0.1` statt InetAddress-Auflösung" / „3. HTTP-Connector" / **„neue `Human(id,OPERATOR)`- oder `TerminalPrincipal.Operator`-Mint-Stelle ohne Registry-Eintrag → Closure `gefunden≠gepinnt`"** (der 21.-Kante-Zahn) / **„`CpJwtVerifier:36` als COOKIE-Achse fehl-klassifiziert (AAL2 verlangt) → sein device-PoP-Verhaltenszahn rötet"** (fängt falsche Registry-Eintragung) / **„Literal-Grep `Human(OPERATOR)` statt Konstruktor-Enum → verfehlt berechnetes `:121`"** (Anti-Vakuitäts-Probe: `:121` aus der Registry entfernen und der Test bleibt grün ⟹ er war die Literal-Variante ⟹ Fassung defekt) / **„ein `routing/`-Handler liest `ory_kratos_session` direkt (hand-gerollter Role→Operator)"** → Bein-③(a)-Arch-Assertion ROT / **„ein `routing/`-Handler ruft `IdentityProvider.whoami()` direkt statt via `resolvePrincipal`"** → Bein-③(b)-Arch-Assertion ROT (die subtilere Hälfte) / **„der Cookie-Consumer-Detektor flaggt die bekannten `auth/`-Consumer NICHT"** → positive Vorbedingung von ③ ROT (Anti-Vakuität) / **„eine `isOperator`-Call-Site ungepinnt"** → Bein-② ROT.
...
### 11.1 Build-Gate-Verifikationen (PL-B1/B2 — am Bau zu prüfen, KEIN Re-Design, keine mutations-Zähne)

Zwei Voraussetzungen, ohne die die §9-Eigenschaften selbst fälschbar werden — am Build-Gate zu verifizieren, nicht als Design-Änderung:

- **B1 — AAL2-Signal server-derived:** das AAL/`amr`/`acr`, das `resolvePrincipal` liest, MUSS aus **server-seitiger Kratos-Session-Introspektion** stammen, NIE aus einem client-lieferbaren Feld/Header/Cookie-Claim. Ein client-setzbares AAL macht das AAL2-Gate (Zahn 8a) selbst fälschbar — dieselbe Krankheit wie §9.1s Cookie-Loch, eine Ebene höher (die server-derived-Attributions-Lehre aus CYP-787/779).
- **B2 — statischer `operatorToken` nie im Browser-Kontext:** kein Endpoint liefert ihn, er steht in keiner web-served Config/Bundle — sonst wird die §9.2-Annahme „Token-Achse = kein Browser-Vektor" falsch und die Cookie-vs-Token-Achsentrennung bricht.
...
**Rev.3.3 (2026-07-21, PL-§4b-Freigabe):** A1 in Zahn 7(b) gefaltet (C3-Registry-Partition-Floor, fail-closed ≤TTL) · A2 in Zahn 8(a) gefaltet · §11.1 Build-Gate-Verifikationen B1/B2 · §14 Bau-Kopplungen (CYP-702 Noise-Auflage) + benannte Konsequenzen. Additiv, kein Modell-Change.

**Rev.3.4 (2026-07-21, Reviewer-Zahn-Verifikation):** Reviewer GO auf 7(b)/B1/B2/§14; **EIN Fund auf 8(a)** — „resolvePrincipal ODER isOperator" ist ein Achsen-Bild, aber Operator-Autorität ist ein **Typ**-Konstrukt (≥3 Typen, ≥5 Mint-Stellen, berechnete Rolle) → verfehlt/falsch-klassifiziert `CpJwtVerifier.kt:36`. 8(a) auf **Konstruktions-Stellen-Closure** umgeschrieben (`gefunden==gepinnte Registry` + per-Achse-Verhalten; hand-listen-frei am Typ-Konstruktor) + gepinnte Baseline-Registry + Non-Vakuitäts-Mutationen (neue-Kante / Fehlklassifikation / Literal-Grep-Probe). 7(b) `exp`-Floor-Schärfung (registry-unabhängiger passiver `NOT_EXPIRED`, kein Re-Mint). Additiv.

**Rev.3.5 (2026-07-21, Reviewer-Re-Pass-Completeness-Fund):** Reviewer gab Faithfulness-GO auf die 8(a)-Closure-Umschreibung, fand beim Re-Verifizieren aber eine **Completeness-Lücke in der eigenen Formulierung** (ehrlich): Typ-Konstruktions-Closure ist **notwendig, nicht hinreichend** — `isOperator(token)` ist ein Boolean (kein Typ, ①-blind), und der echte 21.-Kanten-Cookie-Seizure ist eine `routing/`-Fläche, die Operator aus Session/Cookie OHNE `resolvePrincipal` bestimmt (hand-gerollter Role-Check ODER direkter `IdentityProvider.whoami()`-Call) → unsichtbar für Typ-Closure UND AAL2-Gate. 8(a) auf **DREI Beine** gehärtet: ① Typ-Closure (wie 3.4) · ② `isOperator`-Call-Site-Pin (Token-Achse) · ③ Single-Cookie-Consumer-Arch-Assertion (rohes Cookie **und** `IdentityProvider.whoami` auf `auth/` konfiniert, null `routing/`, mit positiver Detektor-Vorbedingung gegen still-grünen Detektor). +5 Non-Vakuitäts-Mutationen. Additiv. Reviewer-Final-OK auf 3-Bein-Fassung → PL-Go am Volltext (7b+8a) → Bau.

---

## 14. Bau-Kopplungen & benannte Konsequenzen (PL-getragen, für Bau-Order-Vollständigkeit protokolliert)

**Kopplung → CYP-702-Scope (harte Auflage, nicht optional):** der eigene-Relay-Bau (Q3/CYP-702) MUSS die §9.4-MVP-Grenze wahren — **Relay bleibt Noise-Rendezvous, KEIN HTTP-Reverse-Proxy des Browsers.** Ein HTTP-Proxy-Relay tript §9.4s Bruchbedingung (shared-origin → Cookie-Replay → Seizure) und verlangt §9.6 (per-Hub-Origins) ZUERST. Der PL trägt diese Auflage in den CYP-702-Scope.

**Benannte Konsequenzen (keine Blocker — PL bündelt sie dem Auftraggeber):** (a) auf einem **off-loopback-Hub ist der Browser-Operator-Pfad deaktiviert** (Operator muss CMP/native) — korrekte fail-closed-Posture, aber eine Verhaltensfolge; (b) jedes **neue Hub-Warming braucht einen OOB-Konsolen-Fingerprint-Schritt** (kein Bulk-Auto-Enroll) — korrekte Anti-Seizure-Posture, aber „Hub hinzufügen"-Friction.

**Sequenz:** Reviewer verifiziert die Zahn-Fassung (A1/A2) → PL-Go auf die Zahn-Fassung → Modell-2-Bau (nach CYP-775-Kette, Backend) + CYP-702-Scope (PO, mit Noise-Auflage). B1/B2 = Build-Gate-Verify. **Kein Bau vor PL-Go auf die Zahn-Fassung.**
