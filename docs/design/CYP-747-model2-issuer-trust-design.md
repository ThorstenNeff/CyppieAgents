...
> Status: **Design komplett (Rev. 3.3 — PL-§4b-FREIGABE erteilt 2026-07-21; A1/A2-Auflagen als §11-Zähne gefaltet, B1/B2 als §11.1-Build-Verify, §14 Bau-Kopplungen). Nächstes Gate: Reviewer verifiziert die Zahn-Fassung → PL-Go auf die Zahn-Fassung → Modell-2-Bau (nach CYP-775-Kette). KEIN Bau vor PL-Go auf die Zahn-Fassung.**
...
7. **★ Aktiver-Widerruf-Wirksamkeit (Q2/a2)** (§7): (a) nach Widerruf → **Push-STALE/REJECTED über ALLE live Hub-Sessions SOFORT** (nicht ≤TTL); CYP-697-Fall (Operator entfernt → Agenten verstummen) explizit. **(b) Registry-Partition-Floor (PL-A1, fail-closed):** fällt die C3-Widerruf-Registry aus / partitioniert, degradiert die Widerruf-Wirksamkeit auf den **≤TTL-lazy-Floor — NIE länger, NIE fail-open**; ein Registry-Ausfall darf die a2-Sofortwirksamkeit nicht über den ≤TTL-Floor hinaus schwächen (sonst untergräbt genau der Ausfall den a2-Zweck). Non-vakuos: Mutation „Registry-down → Widerruf ignoriert (fail-open)" ODER „Registry-down → degradiert über ≤TTL hinaus" → rötet.
8. **★★ Browser-AAL2-Chokepoint-EIGENSCHAFT + Nicht-Fälschbarkeit der local/remote-Grenze** (§9.3/§9.5): (a) eine synthetische **Nicht-AAL2**-Operator-Cookie-Session wird an JEDER operator-autoritativen Cookie-Kante abgewiesen — **strukturell** via **Flächen-Registry-Meta-Test (PL-A2)**, der beweist, dass **JEDE** operator-autoritative Fläche — die ~20 `/api`-Routen (via `authenticatedApi`) **UND** WS (`AgentSocket`) **UND** `TerminalAccess` **UND** MCP/SSE-Streams — ihre Operator-Autorität über `resolvePrincipal` (Cookie-Achse) **ODER** die statische Token-Achse (`isOperator`) auflöst; **KEINE Hand-Liste** (die wäre selbst die Aufrufer-Krankheit eine Ebene höher). Der AAL2-Gate am `resolvePrincipal`-Chokepoint deckt damit alle Cookie-Achsen-Flächen uniform; die Token-Achsen-Flächen tragen ihr eigenes Anti-Seizure (Tunnel-Device-PoP / lokal, §9.2) — NICHT als Cookie-Loch fehldeuten. Die Prosa nennt WS/Terminal (§9.2); der **Zahn** muss sie strukturell abdecken, nicht nur die `/api`-Routen — die 21. Kante versteckt sich genau im Nicht-`authenticatedApi`-Wrapper (WS/SSE/MCP). (b) **Loopback-Bind RUNTIME-Zahn (nicht Build-Default):** Boot/Init mit `HUB_HOST=0.0.0.0` → Browser-AAL2-OPERATOR-Posture DISABLED (fail-closed), via `InetAddress.getByName(host).isLoopbackAddress` (T3-Auflösung, NICHT String-`==127.0.0.1`); pinnt BEIDE — `host="localhost"` → ENABLED (löst loopback) UND `host="0.0.0.0"` → DISABLED — nicht nur den `127.0.0.1`-Fall. (c) **Kein-3.-Ingress-Meta-Test:** genau die 2 loopback-Connectors, kein bare-cookie-fähiger Nebeneingang zu `/api`. Non-vakuos: Mutation „AAL1 erlaubt" / „off-loopback → Browser-OPERATOR trotzdem an" / „String-`==127.0.0.1` statt InetAddress-Auflösung" / „3. HTTP-Connector" / „eine operator-autoritative WS/Terminal/MCP-Fläche löst weder über `resolvePrincipal` noch die Token-Achse auf (Nicht-Standard-Wrapper-Umgehung)" rötet.
...
**Rev.3.3 (2026-07-21, PL-§4b-Freigabe):** A1 in Zahn 7(b) gefaltet (C3-Registry-Partition-Floor, fail-closed ≤TTL) · A2 in Zahn 8(a) gefaltet (Flächen-Registry-Meta-Test deckt WS/Terminal/MCP/SSE strukturell, nicht nur `/api`) · §11.1 Build-Gate-Verifikationen B1/B2 · §14 Bau-Kopplungen (CYP-702 Noise-Auflage) + benannte Konsequenzen. Additiv, kein Modell-Change; Reviewer verifiziert die Zahn-Fassung, dann PL-Go, dann Bau.
