...
|---|----------|----------------|
| **CR1** | Noise library for the JVM actual (no Noise exists in-tree). | **SPIKE CONFIRMED** → **`org.signal.forks:noise-java:0.1.1`** (Signal's Maven-Central fork of the reference southernstorm noise-java, MIT). Wrap it behind `expect ClientNoiseTransport`. **Never hand-roll Noise.** |
...
3. **CR1 — SPIKE DONE (design-prep, throwaway).** Chose **`org.signal.forks:noise-java:0.1.1`** (Signal's Maven-Central fork of southernstorm noise-java; MIT; pure-Java, **no runtime deps** — jaxb-api in its POM is test-only, the NK path runs without it on JDK 25). Ran a real NK handshake: suite `Noise_NK_25519_ChaChaPoly_BLAKE2s` instantiates; `-> e,es` (48B) / `<- e,ee` (48B) → both sides `SPLIT`; **`HandshakeState.getHandshakeHash()` exposes `h` (32B BLAKE2s), identical on both ends** (the mandatory §7 seam is real). Negative check: a hub static ≠ the pinned key → `AEADBadTagException` → handshake breaks (CI-1 misroute/MITM caught). Native (macOS/Linux) + later JS/wasm actuals follow; web bridges to WebCrypto/a wasm Noise build (Team-2). Never hand-roll.
