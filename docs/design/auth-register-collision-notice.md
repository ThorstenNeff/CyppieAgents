...
- **Neue testTags (2):** `auth.verify.toLogin`, `auth.verify.toForgot` (Testers kanonische `to<Target>`-Konvention, Dev-fixiert+Tester-QA-bestätigt 2026-07-07 — konsistent zu den bestehenden `auth.login.toRegister`/`auth.register.toLogin`/`auth.forgot.toLogin`; scopeId `verify`, nur bei Register-Ankunft gerendert; Test-Contract v0.5 §2). **Reuse:** `auth.verify.pending` (Body-Node), `auth.verify.email`. *(Vorherige Vorschlagsnamen `signIn`/`reset` durch die kanonische `to<Target>`-Konvention ersetzt.)*
