...
- **Anti-Drift:** der Spec-Typ ist **non-null**, weil Backend2 unter (A) nie null emittiert — sonst sähe Dev5 `number|null` und baute einen toten `=== null`-Zweig gegen eine Fläche, die ihn nie liefert. Dev5s `channelUnread()` (`entry===undefined → unknown`) matcht (A) bereits; sein Modell-Typ folgt dem gepinnten non-null `:core`-Typ.
