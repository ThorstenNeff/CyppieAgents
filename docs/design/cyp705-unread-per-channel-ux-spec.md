...
**UNKNOWN-Kodierung (die Drei-Zustands-Angel — muss der Carrier tragen):** der Client muss **UNKNOWN** (kein Server-Cursor) von **confirmed-read** (`unreadCount=0` **mit** Cursor) unterscheiden können. **UNKNOWN ⟺ kein Server-Cursor**, und das hat **zwei** Kodierungen, die `ChannelReadState{lastReadSeq, unreadCount}` (mit `lastReadSeq: number | null`) beide zulässt:
1. **Kanal fehlt** in der Read-State-Antwort ⇒ UNKNOWN.
2. **Kanal vorhanden, aber `lastReadSeq === null`** (kein Cursor für diesen Prinzipal) ⇒ **ebenfalls UNKNOWN** — ein `unreadCount` ohne Cursor ist **kein** „unread of record" (es gäbe keine gelesene Basislinie; „N unread" ohne Cursor wäre dieselbe unknown-als-Count-Lüge auf Wire-Ebene).
- **Ratifizierungs-Bedingung (①):** ein `unreadCount` ist **nur mit `lastReadSeq !== null` autoritativ**. **Empfehlung: Server OMIT-tet cursorlose Kanäle** (⇒ absent=unknown, eine Regel, Modell bleibt dead-simple wie heute). **Falls** Backend2 lieber alle Kanäle inkludiert, muss Dev5s `channelUnread()` **vor** dem Count-Check ein `entry.lastReadSeq === null → { kind: 'unknown' }` ergänzen (heute mappt es nur `entry === undefined → unknown`). Beides schließt die Naht; eines davon **bilateral fixieren**.
- **vorhanden mit `unreadCount=0` UND `lastReadSeq !== null` ⇒ confirmed-read** (nichts). Ohne diese Unterscheidbarkeit kollabieren §0-Zustände 1+2 → verboten.
...
- **① Carrier — RATIFIZIERT (2026-07-18, UIUX2 ⇄ Backend2, converged): standalone `GET /api/read-state`.** Begründung: Read-State ist **volatil** (jede Nachricht bewegt `unreadCount`, jedes Lesen `lastReadSeq`) — additiv auf dem **stabilen** `/api/channels` würde entweder den Kanal-List-Refetch an Read-State koppeln oder `/api/channels` volatil machen. Standalone + der standalone `ReadStateEvent` (③) bilden einen **kohärenten Read-State-Kanal**. **Ratifizierungs-Bedingung (honesty-kritisch):** **UNKNOWN ⟺ kein Cursor** = Kanal fehlt **oder** `lastReadSeq === null`; `unreadCount` autoritativ **nur mit `lastReadSeq !== null`** (§2). Empfehlung: Server omit-tet cursorlose Kanäle; sonst Dev5-Modell-Tweak (§2). Diese eine Naht vor Server-Bau **bilateral fixieren**.
...
**Status:** **①③ RATIFIZIERT (2026-07-18, converged mit Backend2), ②④⑤ gefoldet.** Eine offene bilaterale Naht vor Server-Bau: die UNKNOWN-Kodierung (omit vs. null-Cursor, §2/①) — Backend2 wählt, Dev5 richtet `channelUnread()` danach aus. Danach: Backend2 baut Server, Dev5 den Cursor-Pfad.
