...
> Konvention (verifiziert gg. `app/shared/.../composeResources/values/strings.xml`, develop `608c8ad`):
...
> **`%1$s` = der vom Backend gemeldete Provider-/Werkzeugname** (z. B. „Claude") — **kein** Secret, **kein**
> Endpoint/Token, nur ein neutrales Anzeigelabel (Spec §5.5). Der Wert kommt aus `Agent.provider`
> (zurückgehalten bis CYP-137-Backend, §-Ask 1) — die UI rendert ihn, sie erfindet ihn nie.
>
...
## Reuse (bestehende Keys — NICHT neu anlegen; verifiziert @ `608c8ad`)
...
- **Kollision:** `provider`/`anbieter`/`vendor` existieren **nicht** in `values/strings.xml` **noch** in
  `values-en/strings.xml` @ `608c8ad` → **0 Kollision** (DE+EN). Die `connector_provider*`-Keys sind im
