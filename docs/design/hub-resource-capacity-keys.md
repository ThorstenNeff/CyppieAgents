...
> im a11y ist Teil der Bedeutung (keine harte SLA-Zahl). Kapazität **unbekannt** ⇒ Readout **absent** (`null≠0`, Q3) —
> kein Key für „unbekannt". **`hubcap_full` (Q2)** tönt WARN-Amber erst bei `current == estimatedMax`.
...
## Self-Validation
- **6 neue Keys**, alle DE+EN befüllt, gleiche Argument-Anzahl je Sprache: 4 `hubcap_*` + 2 `a11y_hubcap_*` = **6**.
- **Argument-Keys:** `hubcap_readout` und `a11y_hubcap_readout` tragen je **genau zwei** (`%1$s`/`%2$s`); alle übrigen
  **0 Arg**. DE/EN-Argument-Anzahl identisch.
