...
6. **Offline/Stale:** bricht `/ws/comm` ab, ist die Matrix **möglicherweise veraltet** → ehrlicher Banner **Reuse `comm_status_offline`** (CYP-17, no-arg „evtl. nicht aktuell" — Team-Standard, §A2). Editieren während offline: entweder sperren oder nur mit deutlichem „unbestätigt"-Zustand — **nie** als enforced anzeigen, solange der WS-Spiegel nicht steht.
