...
>
> **Stand: gegengeprüft gegen `origin/develop` `3990cee5` (2026-07-18).** Der Runbook entstand am 2026-07-14; seither haben
> sich Tunnel-Pool (CYP-610/611/616), Desktop-OIDC (CYP-576/474/578) und die Gateway-Fläche (CYP-638 S0–S7) bewegt.
> **Korrigiert** wurden §0 (Gateway-Abgrenzung), §4 (RFC-8252-Flow + Port-Freigabe) und §5.5 (Pool-Partition).
> **Re-verifiziert und unverändert korrekt:** alle §2-Env-Vars, die INERT-Logzeile (`NoiseRelayConnector.kt:99`,
> wörtlich wie zitiert), Relay-Default `0.0.0.0:8788`, der Loopback-Callback `127.0.0.1:47472/callback`, §1.4 `/api/cp/hubs`.
...
> **Abgrenzung Gateway (CYP-638, seit 07-16/17 gebaut):** der Gateway ist der **browser-seitige** Same-Origin-Front-Door
> (A2, eigener Prozess) und steht **NICHT** im Pfad dieses Dogfoods — der Desktop-Client spricht Relay→Tunnel, nicht den
> Gateway. Er bindet per Default **`127.0.0.1:8080` (loopback-only)**; eine öffentliche Exposition ist deploy-eigen und
> zusätzlich an S5+S6 gebunden. Für diesen Runbook also bewusst außer Betracht — nicht vergessen, sondern abgegrenzt.
...
loopback** (`app/desktopApp/…/main.kt` binds `127.0.0.1:47472`, Handler `/callback`). This is the patch to verify on staging.

> **Wer den Flow treibt (CYP-474 §4 / CYP-576):** Desktop-nativ ist die **RFC-8252-Loopback-Variante** — der **System-Browser**
> fährt den *kompletten* Kratos-Flow (Init → GitHub → Callback → `return_to`), die App treibt ihn **nicht** mehr client-seitig
> vor. Genau das war der CYP-576-Defekt: der App-Client holte die `ory_kratos_continuity`-Cookie in seine eigene Jar, der
> Browser schloss den Callback ohne sie ab → Kratos 400. **Konsequenz für §4.1:** der Return-URL-Eintrag ist nicht Kosmetik,
> er ist der einzige Weg, auf dem der Browser zurückfindet.
>
> **Port-Freigabe (CYP-578):** die VM gibt Port 47472 auf **jedem** Ausgang frei (Timeout / Fehler / Cancel / Teardown),
> nicht nur beim erfolgreichen Callback — ein Login-**Retry** bindet also sauber neu. Bei einem hängenden Versuch im
> Dogfood: erst Retry, nicht sofort Prozess-Neustart.
...
| 5.5 | **N-tunnel presence** | the hub is present at the relay on the epoch-derived **set** of rendezvous-ids (cap = `DEFAULT_TUNNEL_POOL_CAP` = **24**, CYP-611); the client dials the set minus the base id | per-operator tunnel CAP is the structural DoS floor |
| 5.6 | **Pool-Partition greift (CYP-610)** | im Pool-Log: die 14 langlebigen WS halten je einen eigenen Tunnel, REST teilt sich **einen** Socket | `WS_RESERVED_SLOTS=14` (7 Agent-WS + 7 Singleton-WS — 1 Tunnel = 1 SOCKET, kein Mux/RR8) + `REST_DEDICATED_CONNS=1` (REST kurz+sequentiell über HTTP/1.1-keep-alive, client-seitig als connections-per-route gekappt) ⇒ REST kann **nie** einen WS-Slot nehmen. Bei cap 24: 23 nutzbar = 1 REST + 14 WS + ~8 Headroom (trägt bis ~16 Agenten). **Ohne** diese Partition war es der 1-oben/6-churnen-Bug. |
| 5.7 | **Break-glass-Lane frei (CYP-616)** | Lifecycle-REST (Start/Stop) funktioniert **auch** während einer WS-Churn-Sättigung | `TunnelLane{DATA,CONTROL}`: der Transport routet die Lane **nach Port** (`restAcceptor`→CONTROL, `wsAcceptor`→DATA); CONTROL hat reserviertes Headroom, das DATA nie aufbraucht. Symptom ohne die Lane: `EXHAUSTED@set held=23 inUse=23` → „Server unreachable" beim Stop. |
