...
> QA / Test Engineer (Team2) · 2026-07-10 · Basis: `origin/develop` @ `b3a5870` · proaktiver Pass, kein Auftrag
> Vorgänger: `QA-VACUUM-SWEEP.md` (Abwesenheit ohne Vorbedingung). Dies ist die **andere Richtung**.
>
> **Basis geprüft, nicht angenommen:** `git merge-base --is-ancestor origin/develop HEAD`. Der erste Anlauf
> dieses Sweeps hing an einem lokalen `develop`, das zehn Merges zurücklag. Aufgefallen ist es nur, weil das
> Gate 1365 statt 1513 Tests zählte. *Ein Branch, dessen Basis man nicht geprüft hat, ist derselbe Fehler wie
> ein Testreport, dessen Herkunft man nicht kennt.*
...
Commit `ed6e384`, Parent `b3a5870` (`origin/develop`).
...
:app:shared:jvmTest :app:shared:wasmJsBrowserTest :app:shared:jsBrowserTest   --rerun-tasks
  ...
  app/shared/jvmTest           795 Tests, 0 rot
  app/shared/wasmJsBrowserTest 195 Tests, 0 rot
  app/shared/jsBrowserTest     185 Tests, 0 rot
  SUMME                       1541 Tests, 0 rot     alle Reports frisch
```

*(1541 statt der 1513 des ersten Laufs: CYP-359 ist zwischenzeitlich nach `develop` gemergt. Die Differenz ist
erklärt — eine unerklärte Zahl wäre ein Befund, kein Beleg.)*
