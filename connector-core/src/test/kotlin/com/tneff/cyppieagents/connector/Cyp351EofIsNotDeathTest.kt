...
class Cyp351EofIsNotDeathTest {
    ...
    private class RecordingObserver : SessionObserver {
        ...
        override fun onProcessExit(agentId: String, sessionId: String?, exitCode: Int?) { exits.incrementAndGet() }
    ...
    private fun spawn(cmd: String): Pair<Process, AgentProcess> {
        ...
        val ap = object : AgentProcess {
            ...
            // CYP-351 (rebase): model the real ProcessBuilderSpawner — the observed death BLOCKS on waitFor and
            // yields the real status. Without this the default `awaitExitCode()=null` returns at once, so the
            // reader would report a death at EOF while the process is still alive — the very bug T7 guards.
            override suspend fun awaitExitCode(): Int = withContext(Dispatchers.IO) { p.waitFor() }
        }
        return p to ap
    }
