...
/**
 * CYP-377 — **the shared content-window floor carries the COMM window too, not only the Agent window it was
 * derived from.**
 *
 * [CONTENT_WINDOW_MIN_HEIGHT] (265 dp) governs every content window — `AgentShell` classes Agent **and Comm** as
 * content (both carry a composer). But `ContentWindowChromeFloorGuardTest` measures only the `AgentWindow`
 * composition, and the number came out of Agent chrome. If the Comm window's chrome were taller, the floor would
 ...
 * sits under it:
 *  - Comm's title bar is **36 dp**, not the Agent's 64 dp (measured on the real shell — the frame is not the same
 *    height for both windows), so at the 265 floor CommPanel receives 265 − 36 = **229 dp**.
 *  - Comm's own fixed chrome (conversation header + composer) is ~133 dp; the message list is the `weight(1f)`
 *    middle. So at 229 dp the composer is whole and the message area keeps ~96 dp — nowhere near the all-chrome
 *    clamp the Agent window hit before CYP-363.
 *
...
class Cyp377CommWindowFloorFitTest {
    ...
    fun theCommComposerSurvivesTheSharedFloor_soTheFloorCarriesBothContentWindows() {
        ...
        listOf(
            ...
            assertTrue(
                ...
                "CYP-377 [$name=$windowFloor]: at the shared floor the Comm composer input is only " +
            ...
            assertTrue(
                ...
                "CYP-377 [$name=$windowFloor]: at the shared floor the Comm message area is gone — the window is " +
