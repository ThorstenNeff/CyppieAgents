...
/**
 * Web (Kotlin/Wasm): TOFU pins persist in `window.localStorage`, a synchronous browser KV (the same store
 * [com.tneff.cyppieagents.ui.ThemePreferences] uses, `cyppie.`-prefixed; a `js()` function references its
 * parameters by name). The Base64 mapping + fail-safe decode live in commonMain ([PersistentPinnedHubStore]);
 * this only wires the raw keyed get/set/remove. [rawGet] returns `""` for an unset key (undefined-safe `|| ''`),
 * treated by the commonMain decode as "not pinned" — never a crash.
 */
